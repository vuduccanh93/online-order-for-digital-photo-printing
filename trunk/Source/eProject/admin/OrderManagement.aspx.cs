...
public partial class admin_OrderManagement : System.Web.UI.Page
{
    ...
    {
        SqlConnection con = DBConnection.getConnection();
        SqlCommand cmd = new SqlCommand("SELECT * FROM Orders WHERE OrderTime BETWEEN @date1 AND @date2 ORDER BY OrderTime DESC", con);
        cmd.Parameters.Add("@date1", txtDate1.Text);
        cmd.Parameters.Add("@date2", txtDate2.Text);
        SqlDataReader reader = cmd.ExecuteReader();
        GridView1.DataSource = reader;
        GridView1.DataBind();
        foreach (GridViewRow row in GridView1.Rows)
